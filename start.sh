...
function create_master(){
    ...
    get_ipv6_node_cidr_gw_addr ipv6_node_cidr_gw_addr "${MASTER_IPV4}"
...
EOF
            ...
            if [ -n "${DUAL_STACK}" ]; then
                ...
                if [[ "${IPV4}" -ne "1" ]]; then
                    ...
                    # write_master_route "${master_prefix_ip}" "${master_ipv6_node_cidr}" \
